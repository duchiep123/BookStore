 ...
 * @author mrhie
 */
public class ShowOrderDetailHistoryServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        response.setContentType("text/html;charset=UTF-8");
         try {
            ...
            Vector<OrderDetailDTO> listDetail = daoOrderDetail.getOrderDetailByOrderID(orderID);
            request.setAttribute("LISTDETAIL", listDetail);

        } catch (Exception e) {
            log("Error at SearchOrderServlet: " + e.getMessage());
        } finally {
            request.getRequestDispatcher("historydetailpage.jsp").forward(request, response);
        }
    }
